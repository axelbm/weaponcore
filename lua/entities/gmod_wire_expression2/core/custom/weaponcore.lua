...
-----
e2function void entity:plySetActiveWeapon(string weapon)
	if not ValidPly(this) then return end
	if not hasAccess(self.player) then return end

	local weap = getWeaponByClass(this, weapon)
	if not weap:IsWeapon() then return end

	this:SetActiveWeapon(weap)
end

e2function void entity:plySetActiveWeapon(entity weapon)
	if not ValidPly(this) then return end
	if not hasAccess(self.player) then return end
...
-----
e2function void entity:plyStripWeapons(string weapon)
	if not ValidPly(this) then return end
	if not hasAccess(self.player) then return end
...
e2function void entity:plyStripWeapons(entity weapon)
	if not ValidPly(this) then return end
	if not hasAccess(self.player) then return end
